...
An upstream failure is a `502`, or a `504` when it was a timeout, with one `X-Tip-Error` naming the kind: `dial`, `tls`, `timeout`, `proxy`, `proxy_rejected` or `read`. When the proxy named in `X-Tip-Proxy` refuses the CONNECT, `X-Tip-Proxy-Status` carries its raw status code, `X-Tip-Proxy-Error` the first line of its body, and its own headers are relayed, so a vendor's `407` and its error header reach you as the vendor sent them. A failure before the tunnel is up names its phase, `proxy-dial`, `proxy-tls` or `proxy-connect`; anything after it is the origin's. TIP removes any `X-Tip-*` header an origin sends, so those headers are always TIP's own. Every response TIP generates also carries an RFC 9209 `Proxy-Status` entry named `tip`, with a typed `error`, the `next-hop` it was talking to and, for a refused CONNECT, the hop's code as `received-status`; an origin's own `Proxy-Status` passes through untouched, so the `X-Tip-*` headers are the ones to trust. Credentials never travel in any of this: the userinfo of any URL inside an error message, a hop's body line or a build failure is replaced with `***` before it reaches a header or the log. A missing or wrong credential for TIP itself is a `407`.
